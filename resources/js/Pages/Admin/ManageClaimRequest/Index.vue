...
watch([search, statusFilter, typeFilter], ([newSearch, newStatus, newType], [oldSearch, oldStatus, oldType]) => {
    if (newSearch !== oldSearch || newStatus !== oldStatus || newType !== oldType) {
        router.get(route("manage.claim-request.index"), {
            search: newSearch,
...
const viewClaim = (claim) => {
    router.get(route('manage.claim-request.show', claim.id));
};

const approveClaim = (claim) => {
    confirm.require({
        message: `Are you sure you want to approve this ${claim.type_display} from ${claim.user.name}?`,
        header: "Approve Claim Confirmation",
        icon: "pi pi-check",
        acceptClass: "p-button-success",
        accept: () => {
            router.put(route('manage.claim-request.update', claim.id), {
                action: 'approve'
            }, {
                preserveState: false,
                preserveScroll: true,
...
const rejectClaim = (claim) => {
    confirm.require({
        message: `Are you sure you want to reject this ${claim.type_display} from ${claim.user.name}?`,
        header: "Reject Claim Confirmation",
        icon: "pi pi-times",
        acceptClass: "p-button-danger",
        accept: () => {
            router.put(route('manage.claim-request.update', claim.id), {
                action: 'reject',
                notes: 'Claim rejected by administrator'
            }, {
                preserveState: false,
                preserveScroll: true,
                onSuccess: () => {
    ...
    router.post(route('manage.claim-request.bulk-action'), {
        ...
        onSuccess: () => {
            toast.add({
                ...
                detail: `${selectedClaims.value.length} claim(s) ${bulkAction.value}d successfully`,
                life: 3000,
            });
    ...
    <AppLayout>
        ...
        <div class="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-6">
                ...
                <div v-for="claimType in claimTypes" :key="claimType.id" 
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div :class="`p-2 rounded-lg bg-${claimType.bgColor}`">
                                <i :class="`${claimType.icon} text-${claimType.color} text-lg`"></i>
                            </div>
                            <div>
                                <h3 class="font-semibold text-gray-900 text-sm">{{ claimType.name }}</h3>
                                <p class="text-xs text-gray-500 mt-1">{{ claimType.description }}</p>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
                ...
                <Card class="border-l-4 border-yellow-500 shadow-sm">
                    <template #content>
                        <div class="text-center">
                ...
                <Card class="border-l-4 border-gray-500 shadow-sm">
                    <template #content>
                        <div class="text-center">
            ...
            <Card class="shadow-lg">
                <template #content>
                    ...
                    <div class="flex flex-col gap-3 sm:gap-4">
                        ...
                        <!-- Selected Claims Info -->
                        <div v-if="selectedClaims.length > 0" class="p-2 sm:p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <Button label="Clear" icon="pi pi-times" severity="secondary" text size="small"
                                    @click="selectedClaims = []" class="text-xs" />
                            </div>
                        </div>
                    </div>
                        ...
                        <DataTable :value="claims.data" showGridlines stripedRows
                            ...
                            <template #empty>
                                <div class="flex flex-col items-center justify-center py-8 sm:py-12">
                                    ...
                                    <p class="mb-4 text-xs sm:text-sm text-gray-500 text-center px-2">No claim requests match your search criteria.</p>
                                </div>
                            </template>
                            ...
                            <Column field="user.name" header="User" sortable style="min-width: 180px;">
                                <template #body="slotProps">
                                    <div class="font-semibold text-gray-900 text-xs sm:text-sm break-words">{{ slotProps.data.user.name }}</div>
                                    <div class="text-xs text-gray-500 break-all">{{ slotProps.data.user.email }}</div>
                                </template>
                            </Column>
                            ...
                            <Column field="purpose" header="Purpose" sortable style="min-width: 200px;">
                                <template #body="slotProps">
                                    <div class="text-xs sm:text-sm text-gray-900 line-clamp-2">{{ slotProps.data.purpose || 'No purpose specified' }}</div>
                                    <div v-if="slotProps.data.details" class="text-xs text-gray-500 mt-1">
                                        <span v-for="(value, key) in slotProps.data.details" :key="key" class="mr-2">
                                            {{ value }}
                                        </span>
                                    </div>
                                </template>
                            ...
                            <Column header="Submitted" sortable style="min-width: 120px;">
                                <template #body="slotProps">
                                    <div class="text-xs sm:text-sm text-gray-600">
                                        {{ formatDate(slotProps.data.submitted_at) }}
                                    </div>
                                </template>
                            </Column>
                            ...
                            <Column header="Actions" style="min-width: 140px">
                                <template #body="slotProps">
                                    <div class="flex gap-1 flex-wrap">
                                        ...
                                        <Button v-if="slotProps.data.status === 'submitted'" 
                                            icon="pi pi-check" outlined rounded severity="success" size="small"
                                            v-tooltip.top="'Approve Claim'" @click="approveClaim(slotProps.data)" 
                                            class="w-7 h-7 sm:w-8 sm:h-8 p-0" />

                                        <Button v-if="slotProps.data.status === 'submitted'" 
                                            icon="pi pi-times" outlined rounded severity="danger" size="small"
                                            v-tooltip.top="'Reject Claim'" @click="rejectClaim(slotProps.data)" 
                                            class="w-7 h-7 sm:w-8 sm:h-8 p-0" />
                                    </div>
                                </template>
                            </Column>
