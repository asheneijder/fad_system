    ...
    <AppLayout>
        ...
        <div class="p-6 card">
            <div class="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
                <Button label="Create New User" icon="pi pi-plus" class="p-button-success p-button-sm"
                    @click="createUser" />
                <InputText v-model="search" placeholder="Search users..." class="w-full p-inputtext-sm sm:w-64" />
            </div>

            <DataTable :value="users.data" showGridlines :rowHover="true" :loading="users.loading" paginator
                :rows="users.per_page" :totalRecords="users.total" :first="(users.current_page - 1) * users.per_page"
                lazy @page="onPageChange" tableStyle="min-width: 50rem" responsiveLayout="scroll">
                <!-- Empty state -->
                <template #empty>
