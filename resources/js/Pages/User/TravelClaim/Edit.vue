...
const items = [
    ...
    { label: 'Travel Claim #' + props.travelClaim.id, url: route('user.travel-claims.show', { travelClaim: props.travelClaim.id }) },
    { label: 'Edit Claim' }
];
...
// Initialize travel legs from existing data - FIXED: Ensure proper initialization
const initializeTravelLegs = () => {
    if (props.travelClaim.travel_legs && Array.isArray(props.travelClaim.travel_legs) && props.travelClaim.travel_legs.length > 0) {
        travelLegs.value = props.travelClaim.travel_legs.map(leg => ({
            from: leg?.from || '',
            to: leg?.to || '',
        ...
        // Fallback to single leg if no legs data
        travelLegs.value = [
            { 
                from: props.travelClaim.travel_from || '', 
                to: props.travelClaim.travel_to || '', 
                date: props.travelClaim.date_of_travel ? new Date(props.travelClaim.date_of_travel) : null, 
                distance: props.travelClaim.total_distance || null 
            }
        ];
    }
};

// FIX: Call initialization immediately and also in onMounted for safety
initializeTravelLegs();

// FIX: Use onMounted properly with null check
onMounted(() => {
    // Re-initialize to ensure data is loaded
...
// Remove travel leg with safety check
const removeTravelLeg = (index) => {
    if (travelLegs.value.length > 1 && index >= 0 && index < travelLegs.value.length) {
        travelLegs.value.splice(index, 1);
    }
};

// Calculate total distance from all legs with safety checks
const totalDistance = computed(() => {
    return travelLegs.value.reduce((sum, leg) => {
        if (leg && typeof leg.distance === 'number') {
            return sum + leg.distance;
        }
        return sum + (parseFloat(leg?.distance) || 0);
    }, 0);
});

const form = useForm({
    vehicle_type: props.travelClaim.vehicle_type || 'car',
    registration_plate_number: props.travelClaim.registration_plate_number || '',
    cubic_capacity: props.travelClaim.cubic_capacity || null,
    is_multiple_legs: props.travelClaim.is_multiple_days || false,
    travel_legs: [],
    purpose: props.travelClaim.purpose || '',
    total_distance: props.travelClaim.total_distance || 0,
    rate_per_km: props.travelClaim.rate_per_km || props.defaultRates.car || 0.60,
    attachments: [],
    save_as_draft: false,
});
...
// Update travel legs in form data with safety checks
watch(travelLegs, (newLegs) => {
    form.travel_legs = newLegs.map(leg => {
        if (!leg) return { from: '', to: '', date: null, distance: 0 };
...
const removeFile = (index) => {
    if (index >= 0 && index < form.attachments.length) {
...
const removeDocument = (index) => {
    // This would require an API call to delete the media
    toast.add({
        severity: 'info',
        summary: 'Document Removal',
        detail: 'Please contact administrator to remove existing documents',
        life: 3000
    });
};
...
const submitFormWithAgreement = (saveAsDraft = false) => {
    ...
    form.put(route('user.travel-claims.update', { travelClaim: props.travelClaim.id }), {
        onSuccess: () => {
    ...
    form.put(route('user.travel-claims.update', { travelClaim: props.travelClaim.id }), {
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Success',
                detail: 'Travel claim saved as draft successfully!',
                life: 5000
            });
        },
        onError: (errors) => {
...
// Validate form before submission with safety checks
const validateForm = () => {
    // Check vehicle type
    ...
    for (const leg of travelLegs.value) {
        if (!leg || !leg.from || !leg.to || !leg.date || !leg.distance) {
            return false;
        }
    }
...
const getRateDisplayText = computed(() => {
    const rates = {
        car: '0.75 RM/km (Car)',
        motorcycle: '0.45 RM/km (Motorcycle)',
    };
    return rates[form.vehicle_type] || '0.50 RM/km';
});

// Get route summary for display with safety checks - FIXED: This was line 304
const routeSummary = computed(() => {
    if (!travelLegs.value.length) return '—';
    
    if (travelLegs.value.length === 1) {
        const leg = travelLegs.value[0];
        return `${leg?.from || '—'} → ${leg?.to || '—'}`;
    } else {
        const locations = travelLegs.value
            .map(leg => leg?.from)
            .filter(Boolean);
...
// Check if a leg is valid with safety checks
const isLegValid = (leg) => {
    return leg && leg.from && leg.to && leg.date && leg.distance;
};
...
// View claim details
const viewClaim = () => {
    router.get(route('user.travel-claims.show', { travelClaim: props.travelClaim.id }));
};
</script>

<template>
    <Head :title="`Edit Travel Claim #${travelClaim.id}`" />
    <AppLayout>
        <Toast />
        ...
        <div class="p-3 sm:p-4 md:p-6 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Travel Claim #{{ travelClaim.id }}</h1>
                    <p class="mt-1 text-sm sm:text-base text-gray-500">Update your travel expense claim</p>
                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <InputText v-model="form.registration_plate_number" placeholder="e.g., ABC1234"
                                        class="w-full" />
                                    <small class="text-red-500 text-xs" v-if="form.errors.registration_plate_number">{{ form.errors.registration_plate_number }}</small>
                                </div>
                    ...
                    <Card class="shadow-lg">
                        ...
                        <template #content>
                            <div class="space-y-4">
                                ...
                                <div class="space-y-2">
                                    <label class="block text-sm font-medium text-gray-700">Upload Additional Documents</label>
                        ...
                        <template #content>
                            <div class="space-y-3">
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-sm font-medium">#{{ travelClaim.id }}</span>
                                </div>
                                <div class="flex justify-between">
                        ...
                        <template #content>
                            <div class="space-y-2">
                                ...
                                <ul class="text-xs text-gray-600 space-y-1 list-disc list-inside">
                                    <li>You can modify any travel leg details</li>
                                    <li>Add or remove travel legs as needed</li>
