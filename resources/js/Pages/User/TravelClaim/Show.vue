...
const printClaim = () => {
    // Create a print-friendly version of the claim
    const printWindow = window.open('', '_blank');
    const printContent = `
        <!DOCTYPE html>
        <html>
        <head>
            <title>Travel Claim #${props.travelClaim.id}</title>
            <style>
        ...
        <body>
            <div class="header">
                <h1>Travel Claim #${props.travelClaim.id}</h1>
                <p>Generated on ${new Date().toLocaleDateString('en-MY')}</p>
                <p><strong>Status:</strong> ${getStatusText(props.travelClaim.status)}</p>
            </div>
            ...
            <div class="section">
                ...
                <div class="grid">
                    <div class="field">
                        ...
                        <div class="field-value">${props.travelClaim.user?.name || '—'}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Department</div>
                        <div class="field-value">${props.travelClaim.user?.department || '—'}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Position</div>
                        <div class="field-value">${props.travelClaim.user?.position || '—'}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Employee ID</div>
                        <div class="field-value">${props.travelClaim.user?.employee_id || '—'}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="grid">
                    <div class="field">
                        ...
                        <div class="field-value">${vehicleTypeLabel.value}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Registration Plate</div>
                        <div class="field-value">${props.travelClaim.registration_plate_number || '—'}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Cubic Capacity</div>
                        <div class="field-value">${props.travelClaim.cubic_capacity ? props.travelClaim.cubic_capacity + ' cc' : '—'}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Rate per KM</div>
                        <div class="field-value">${formatCurrency(props.travelClaim.rate_per_km)}</div>
                    </div>
                </div>
            </div>

            <div class="section">
                <div class="section-title">Travel Journey</div>
                <table>
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        ${travelLegs.value.map((leg, index) => `
                            <tr>
                                <td>${index + 1}</td>
                                <td>${formatDate(leg.date)}</td>
                                <td>${leg.from}</td>
                                <td>${leg.to}</td>
                                <td>${parseFloat(leg.distance).toFixed(1)}</td>
                                <td>${formatCurrency(leg.distance * props.travelClaim.rate_per_km)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                    <tfoot>
                        <tr class="total-row">
                            ...
                            <td><strong>${totalDistanceFromLegs.value.toFixed(1)} km</strong></td>
                            <td><strong>${formatCurrency(totalCostFromLegs.value)}</strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="section">
                ...
                <div style="padding: 15px; border: 1px solid #ddd; border-radius: 4px; margin-top: 10px;">
                    ${props.travelClaim.purpose || 'No purpose specified'}
                </div>
            </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 15px;">
                    <div>
                        <div class="field">
                            ...
                            <div class="field-value">${props.travelClaim.total_distance} km</div>
                        </div>
                        <div class="field">
                            <div class="field-label">Rate per KM</div>
                            <div class="field-value">${formatCurrency(props.travelClaim.rate_per_km)}</div>
                        ...
                        <div class="field" style="margin-top: 20px; padding-top: 10px; border-top: 2px solid #333;">
                            ...
                            <div class="field-value" style="font-size: 20px; color: #3b82f6;">${formatCurrency(props.travelClaim.total_cost)}</div>
                        </div>
                    </div>
                    <div style="background: #f0f9ff; padding: 15px; border-radius: 4px;">
                        <h4 style="margin-top: 0; color: #0369a1;">Calculation</h4>
                        <p>${props.travelClaim.total_distance} km × ${formatCurrency(props.travelClaim.rate_per_km)} = ${formatCurrency(props.travelClaim.total_cost)}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="grid">
                    <div class="field">
                        ...
                        <div class="field-value">${formatDateTime(props.travelClaim.created_at)}</div>
                    </div>
                    <div class="field">
                        <div class="field-label">Claim Date</div>
                        <div class="field-value">${formatDate(props.travelClaim.claim_date)}</div>
                    </div>
                    ${props.travelClaim.approval_date ? `
                    <div class="field">
                        <div class="field-label">Approval Date</div>
                        <div class="field-value">${formatDateTime(props.travelClaim.approval_date)}</div>
                    </div>
                    ` : ''}
                    ${props.travelClaim.approver ? `
                    <div class="field">
                        <div class="field-label">Approved By</div>
                        <div class="field-value">${props.travelClaim.approver.name}</div>
                    </div>
                    ` : ''}
                </div>
            </div>
            ...
            <div class="footer">
                <p>This is an automatically generated travel claim document.</p>
                <p>Document ID: ${props.travelClaim.id} | Generated on: ${new Date().toLocaleString('en-MY')}</p>
            </div>
