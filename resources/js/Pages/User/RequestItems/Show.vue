    ...
    <AppLayout>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">My Request #{{ requestItem.id }}</h1>
                ...
                <div class="flex gap-3">
                    <Button label="Back to My Requests" icon="pi pi-arrow-left" severity="secondary" outlined
                        @click="router.get(route('user.request-items.index'))" />
                    <Button v-if="requestItem.status === 'pending'" 
                        label="Cancel Request" icon="pi pi-times" severity="danger" outlined
                        @click="cancelRequest" />
                </div>
            </div>
                    ...
                    <Card class="shadow-lg">
                        <template #content>
                            ...
                            <div class="space-y-4">
                                ...
                                <div v-if="requestItem.approved_by" class="p-4 bg-green-50 border border-green-200 rounded-lg">
                                    <label class="block text-sm font-medium text-green-700">Approved By</label>
                                    <p class="mt-1 text-green-600">{{ requestItem.approved_by?.name }} on {{ formatDate(requestItem.approved_at) }}</p>
                                </div>
                            </div>
                        </template>
                    </Card>
                                ...
                                <div v-for="item in requestItem.items" :key="item.id" 
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                        ...
                                        <div class="text-center p-3 bg-blue-50 rounded">
                                            <label class="block text-sm font-medium text-gray-500 mb-1">Approved</label>
                                            <p class="font-semibold text-blue-600 text-lg">
                                                {{ item.approved_quantity || item.quantity }}
                                            </p>
                                            <p class="text-xs text-gray-500">{{ item.stationary_item?.unit }}</p>
                                        </div>
                                        <div class="text-center p-3" :class="{
                                            ...
                                            <p class="font-semibold text-lg" :class="{
                                                ...
                                                {{ item.approved_quantity === item.quantity ? 'Fully Approved' : 
                                                   item.approved_quantity ? 'Partially Approved' : 'Pending' }}
                                            </p>
                                            <p class="text-xs text-gray-500">
                                                {{ item.approved_quantity ? item.approved_quantity + ' approved' : 'Waiting approval' }}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div v-if="requestItem.items" class="border-t mt-6 pt-6">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                                    <div>
                                        ...
                                        <span class="block text-2xl font-bold text-gray-900 mt-1">{{ totalItems }}</span>
                                    </div>
                                    <div>
                                        <span class="block text-sm font-semibold text-gray-500">Total Approved</span>
                                        <span class="block text-2xl font-bold text-blue-600 mt-1">{{ totalApprovedItems }}</span>
                                    </div>
                                    <div>
                ...
                <div class="space-y-6">
                    ...
                    <Card class="shadow-lg">
                        <template #content>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-500">Request ID</label>
                                    <p class="mt-1 font-medium text-gray-900">#{{ requestItem.id }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 font-medium text-gray-900">{{ formatDate(requestItem.updated_at) }}</p>
                                </div>
                    ...
                    <Card class="shadow-lg">
                        <template #content>
                            ...
                            <div class="space-y-3">
                                <div v-if="requestItem.status === 'pending'" class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Your request is being reviewed by the admin team.</p>
                                    </div>
                                </div>
                                
                                <div v-if="requestItem.status === 'approved'" class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Your request has been approved and will be processed soon.</p>
                                    </div>
                                </div>
                                
                                <div v-if="requestItem.status === 'completed'" class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Your request has been fulfilled and items have been issued.</p>
                                    </div>
                                </div>
                                
                                <div v-if="requestItem.status === 'rejected'" class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Please review the rejection reason and contact admin if needed.</p>
                                    </div>
                                </div>
                            </div>
                        </template>
