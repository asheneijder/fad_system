...
class SendClaimApprovalNotification implements ShouldQueue
{
     ...
     */
    public function __construct($claim, string $claimType, User $approver)
    {
        $this->claim = $claim;
    ...
    public function handle(): void
    {
        try {
            ...
            Mail::to($user->email)->send(new ClaimApprovedMail($user, $this->claim, $this->claimType, $claimData, $this->approver));

            Log::info("Approval email sent successfully for {$this->claimType} claim ID: {$this->claim->id} to user: {$user->email}");

        } catch (\Exception $e) {
            Log::error("Failed to send approval email for claim ID: {$this->claim->id}. Error: ".$e->getMessage());
        }
    }
